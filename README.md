...
Update personal content directly in the two HTML files. Theme colors and responsive styles live in `assets/academic/style.css`. The supplied portrait is displayed as a circular crop using CSS; the original image is preserved. Add a CV or publication links only once the actual documents are ready; the current version does not invent them.
